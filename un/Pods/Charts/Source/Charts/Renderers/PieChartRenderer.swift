...
open class PieChartRenderer: NSObject, DataRenderer
{
    ...
    private func drawHole(context: CGContext)
    {
        ...
        if chart.drawHoleEnabled
        {
            ...
            if let holeBorderColor = chart.holeBorderColor
            {
                if holeBorderColor != NSUIColor.clear
                {
                    // draw the hole-circle
                    context.setFillColor(chart.holeBorderColor!.cgColor)
                    context.fillEllipse(in: CGRect(x: center.x - holeRadius + 1, y: center.y - holeRadius + 1, width: holeRadius * 2.0 - 2, height: holeRadius * 2.0 - 2))
                }
            }
